...
import {
  ...
  TinkoffFuturesMarginModel,
} from '../../../models/TinkoffFuturesMargin'

const instrumentsBySourceCache = new NodeCache({
  stdTTL: 86400, // 1 day
})

export const getFutureMarginByTickerId = async (
  tickerId: string
): Promise<TinkoffFuturesMargin | null> => {
  ...
  instrumentsBySourceCache.mset(
    futureMargin.map((el) => ({ key: el.tickerId, val: el }))
  )
