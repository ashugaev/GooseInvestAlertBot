...
export const getYahooPrices = async (
  ...
  const {
    data: {
      quoteResponse: { result, error },
    },
  } = await axios(
    `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${yahooRequestSymbols}`
  )
  ...
  if (tickerIds.length < prices.length) {
    const uncheckedTickers = tickerIds.filter(
      (el) => !prices.find((item) => item[0] === el)
    )
