...
export const getBinancePrices = async (
  tickerIds: string[],
  allBinanceInstruments: InstrumentsList[]
): Promise<TickerPrices> => {
  const pricesObj = await binance.prices()

  const pricesArr = Object.entries(pricesObj).map(([key, val]) => [
    key,
    Number(val),
  ])
  ...
  const pricesArrWithId: TickerPrices = pricesArr.reduce<TickerPrices>(
    (acc, [ticker, price]) => {
      const tickerId = allBinanceInstruments.find(
        (el) => el.ticker === ticker
      )?.id
  ...
  if (tickersWithNoPrice.length > 15) {
    log.error(
      logPrefix,
      "Can't find Binance tickers in database for price from API",
      tickersWithNoPrice
    )
  }
...
export const getBinancePricesFutures = async (
  tickerIds: string[],
  allBinanceInstruments: InstrumentsList[]
): Promise<TickerPrices> => {
  const pricesObj = await binance.futuresPrices()
