...
const testTickers: TestItem[] = [
  ...
  {
    params: { source: EMarketDataSources.lbank, ticker: 'ETHUSDT' },
    checkPeriod: 1000 * 60 * 10,
  },
  {
  ...
  for (let i = 0; i < testTickers.length; i++) {
    ...
    const checkPrice = async () => {
      try {
        ...
        const instrumentInfo: InstrumentsList = (
          await InstrumentsListModel.find(itemConfig.params).lean()
        )[0]
        ...
        if (!price) {
          await sayToBoss({
            message: `😱 No price for ${instrumentInfo.ticker} ${getSourceMark(
              instrumentInfo
            )}`,
          })
        }
        ...
        if (price === CHECKED_PRICES[instrumentInfo.id]) {
          await sayToBoss({
            message: `😱 Price for ${instrumentInfo.ticker} ${getSourceMark(
              instrumentInfo
            )} is the same`,
          })
        } else {
