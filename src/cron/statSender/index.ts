    ...
    for (const event of events) {
      let message =
        i18n.t('ru', 'shift_alert_message', {
          ...
          dayOfWeek:
            event.dayOfWeek && i18n.t('ru', `days_${event.dayOfWeek}_2`),
        }) + '\n'

      const { Stock, Etf } = event.data

      Stock &&
        (message += i18n.t('ru', 'shift_alert_message_stock', {
          list: Stock.map(getItemText).join('\n'),
        }))
      Etf &&
        (message += i18n.t('ru', 'shift_alert_message_etf', {
          list: Etf.map(getItemText).join('\n'),
        }))
      ...
      try {
        await (
          await bots
        )[0].telegram.sendMessage(event.user, message, {
          parse_mode: 'HTML',
