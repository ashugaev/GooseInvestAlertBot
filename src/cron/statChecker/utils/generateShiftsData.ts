...
export const generateShiftsData = ({ shifts, instrument, candles }) => {
  ...
  candles.reduceRight(
    (acc, candle, i) => {
      ...
      const sumVolume = candles
        .slice(dayNumber * -1)
        .reduce((acc, candle) => candle.v + acc, 0)
      ...
      shiftValue.growPercent = getPercent({
        initialValue: acc.min,
        diff: currentPrice - acc.min,
      })
      shiftValue.fallPercent = getPercent({
        initialValue: acc.max,
        diff: acc.max - currentPrice,
      })
      shiftValue.sumVolume = sumVolume
      shiftValue.instrument = instrument

      shifts[dayNumber] = shifts[dayNumber] ?? {}
      shifts[dayNumber][instrumentType] =
        shifts[dayNumber][instrumentType] ?? []
