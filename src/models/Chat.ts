...
const isTimeForChatUpdate = (chat) =>
  !lastUpdatedByChatId[chat.id] ||
  Date.now() - lastUpdatedByChatId[chat.id] >= updateChatTimeout
...
export async function createOrUpdateChat(
  ...
  const oldValue = await ChatModel.findOneAndUpdate(
    { id },
    { $set: data },
    { upsert: true }
  )
  ...
  if (
    (isNewChatAdded || isChatActivated) &&
    id &&
    type &&
